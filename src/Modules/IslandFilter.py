...
def allowed_buses() -> Set[str]:
    ...
    Rules:
      - If bus_to_island has already been reduced to one island (GUIâ€™s â€œactivate islandâ€),
    ...
    # No â€œactive islandâ€ filter was applied; keep all sourceful islands
...
def should_comment_branch(from_bus: str | None, to_bus: str | None) -> bool:
    """Comment a 2-terminal device if EITHER endpoint isnâ€™t allowed."""
...
def should_comment_3w(b1: str | None, b2: str | None, b3: str | None) -> bool:
    """Comment a 3-terminal device if ANY endpoint isnâ€™t allowed."""
    return (not is_bus_allowed(b1)) or (not is_bus_allowed(b2)) or (not is_bus_allowed(b3))

# --- Optional pruning policy: remove instead of comment ----------------------

def _remove_mode() -> bool:
    c = _ctx()
    mode = str(c.get("prune_mode", "")).strip().lower()
    return mode in {"remove", "drop", "delete"}


def should_drop_bus(bus_base: str | None) -> bool:
    """If prune_mode == remove, drop rows whose bus is not allowed."""
    return _remove_mode() and should_comment_bus(bus_base)


def should_drop_branch(from_bus: str | None, to_bus: str | None) -> bool:
    """If prune_mode == remove, drop rows whose branch endpoints are not allowed."""
    return _remove_mode() and should_comment_branch(from_bus, to_bus)


def should_drop_3w(b1: str | None, b2: str | None, b3: str | None) -> bool:
    return _remove_mode() and should_comment_3w(b1, b2, b3)

def drop_mode_enabled() -> bool:
    """Expose whether prune_mode is set to remove/drop/delete."""
    return _remove_mode()
